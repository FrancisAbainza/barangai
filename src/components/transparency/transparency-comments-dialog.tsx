...
import {
  addTransparencyProjectComment,
  deleteTransparencyProjectComment,
  getTransparencyProjectComments,
} from "@/actions/transparency";
import type {
  TransparencyProjectCommentWithAuthor,
  TransparencyProjectWithAuthor,
} from "@/actions/transparency";
...
function CommentItem({
  ...
  comment: TransparencyProjectCommentWithAuthor;
  isOwn: boolean;
  onDelete: (id: number) => void;
  onReply?: (comment: TransparencyProjectCommentWithAuthor) => void;
  isReplying?: boolean;
}) {
  return (
...
export default function TransparencyCommentsDialog({
  project,
  open,
  onOpenChange,
}: {
  ...
  const [replyTo, setReplyTo] = useState<TransparencyProjectCommentWithAuthor | null>(null);
  const [replyContent, setReplyContent] = useState("");

  const commentsQueryKey = ["transparency-project-comments", project.id];

  const { data: comments = [], isLoading } = useQuery({
    queryKey: commentsQueryKey,
    queryFn: () => getTransparencyProjectComments(project.id),
    enabled: open,
  });

  const topLevelComments = comments.filter((comment) => comment.parentId === null);
  const repliesByParent = comments.reduce<Map<number, TransparencyProjectCommentWithAuthor[]>>(
    (map, comment) => {
  ...
  const { mutate: postComment, isPending: isPosting } = useMutation({
    mutationFn: ({ text, parentId }: { text: string; parentId?: number }) =>
      addTransparencyProjectComment(project.id, text, parentId),
    onMutate: async ({ text, parentId }) => {
      await queryClient.cancelQueries({ queryKey: commentsQueryKey });
      const previous = queryClient.getQueryData<TransparencyProjectCommentWithAuthor[]>(commentsQueryKey);

      const optimisticComment: TransparencyProjectCommentWithAuthor = {
        id: -Date.now(),
      ...
      queryClient.setQueryData<TransparencyProjectCommentWithAuthor[]>(commentsQueryKey, (old) => [
        ...(old ?? []),
        optimisticComment,
      ]);
  ...
  const { mutate: removeComment } = useMutation({
    mutationFn: (id: number) => deleteTransparencyProjectComment(id),
    onMutate: async (id) => {
      await queryClient.cancelQueries({ queryKey: commentsQueryKey });
      const previous = queryClient.getQueryData<TransparencyProjectCommentWithAuthor[]>(commentsQueryKey);
